...
class SlotAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        for _ in range(self.iters):
            ...
            logits  = torch.einsum('bnd,bsd->bns', k, q) * self.scale
            attn    = torch.softmax(logits, dim=-1)      # [B,N,S]
            updates = torch.einsum('bns,bnd->bsd', attn, v)
            s_flat  = slots.view(-1, self.slot_dim)
            u_flat  = updates.view(-1, self.slot_dim)
            slots   = self.gru(u_flat, s_flat).view_as(slots)
...
def main():
    ...
    # 3) 先跑一个 sample 拿 feat 大小
    sample = next(iter(loader))[0][:1].to(device)  # [1,3,32,32]
    with torch.no_grad():
        feat = f(sample)
    _, C, H, W = feat.shape

    # Attention
    slot_attn  = SlotAttention(num_slots=8, in_dim=C, slot_dim=C, iters=3).to(device)
    cross_attn = CrossAttention(feat_dim=C, slot_dim=C, num_heads=4).to(device)
    slot_attn.eval(); cross_attn.eval()

    # 4) 基线
    correct = total = 0
    with torch.no_grad():
        for imgs, labs in tqdm(loader, desc="Baseline"):
            imgs, labs = imgs.to(device), labs.to(device)
            z    = f(imgs)
            out  = f_tail(z)
            out  = F.adaptive_avg_pool2d(out,1).view(out.size(0),-1)
            pred = cls_head(out).argmax(1)
            correct += pred.eq(labs).sum().item()
            total   += labs.size(0)
    print(f"Baseline Acc: {correct/total*100:.2f}%")

    # 5) 加注意力
    correct = total = 0
    with torch.no_grad():
        for imgs, labs in tqdm(loader, desc="With Attention"):
            imgs, labs = imgs.to(device), labs.to(device)
            z      = f(imgs)
            slots  = slot_attn(z)
            z2     = cross_attn(z, slots)
            out    = f_tail(z2)
            out    = F.adaptive_avg_pool2d(out,1).view(out.size(0),-1)
            pred   = cls_head(out).argmax(1)
            correct+= pred.eq(labs).sum().item()
            total  += labs.size(0)
    print(f"With Attention Acc: {correct/total*100:.2f}%")
